...
// Component to display the latest 6 AI-generated articles on homepage
...
function FeaturedArticle({ article }) {
  return (
    <Link
      ...
      className="group lg:col-span-2 bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all border border-gray-100"
    >
      <div className="relative h-56 bg-gradient-to-br from-blue-50 to-indigo-100">
        ...
        />
        <span
          className={`absolute top-3 left-3 text-xs font-bold px-2.5 py-1 rounded-full ${getCategoryClasses(
            article.category
          )}`}
        >
          {article.category}
...
function SmallArticleCard({ article }) {
  return (
    <Link
      key={article.slug}
      href={`/blog/${article.slug}`}
      className="group flex gap-3 bg-white rounded-xl p-3 hover:shadow-md transition-all border border-gray-100"
    >
      ...
      <div className="flex-1 min-w-0">
        <span
          className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${getCategoryClasses(
            article.category
          )}`}
        >
          {article.category}
        </span>
        <h4 className="text-xs font-semibold text-gray-900 line-clamp-2 mt-1 group-hover:text-blue-600 transition-colors leading-snug">
          {article.title}
        </h4>
