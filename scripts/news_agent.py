...
Directly aitechindia repo mein .md files push karta hai GitHub API se
"""
        ...
        try:
            ...
            for entry in feed.entries[:5]:
                ...
                if title and len(title) > 20:
                    all_articles.append({"source": feed_info["name"], "title": title, "summary": summary, "link": entry.get("link", "")})
            print(f"  OK {feed_info['name']}: fetched")
...
def call_groq(prompt, max_tokens=1000):
    ...
    payload = {"model": GROQ_MODEL, "messages": [{"role": "user", "content": prompt}], "max_tokens": max_tokens, "temperature": 0.7}
...
def choose_topics(articles, count=3):
    print(f"Groq se {count} topics choose karwa raha hoon...")
    articles_text = "\n".join([f"{i+1}. [{a['source']}] {a['title']}" for i, a in enumerate(articles[:20])])
...
[{{"index": 1, "title": "title", "category": "AI Tools", "hindi_angle": "Indian angle"}}]"""
    try:
        resp = call_groq(prompt, 500)
        clean = re.sub(r'```json|```', '', resp).strip()
        topics = json.loads(clean[clean.find('['):clean.rfind(']')+1])
...
def write_article(topic, source):
    print(f"Article likh raha hoon: {topic['title'][:50]}...")
...
{{"title": "Hinglish title under 80 chars", "slug": "url-slug-2026", "excerpt": "2-3 line preview", "content": "full article with \\n\\n paragraphs", "readingTime": "4 min read", "tags": ["AI", "tag2"]}}"""
    try:
        resp = call_groq(prompt, 1800)
        clean = re.sub(r'```json|```', '', resp).strip()
        article = json.loads(clean[clean.find('{'):clean.rfind('}')+1])
...
def push_md_to_github(article, topic, source, image_url):
    if not GITHUB_TOKEN:
...
def main():
    ...
    print(f"\n{len(topics)} articles likhna shuru...")
    published = []
    ...
    print("\n" + "="*55)
    print(f"DONE! {len(published)} articles aitechindia repo pe push!")
    for t in published:
        print(f"  - {t[:60]}")
